...
> **Updated:** 2026-02-24
> **Canonical location:** `bolt-core-sdk/docs/ecosystem/RUST_CORE_PLAN.md`
> **Status:** DRAFT — design review, no production code yet.
> **Scope:** bolt-core-sdk `rust/bolt-core` crate (existing v0.1.1)

Note: The `bolt-ecosystem/` workspace root is not a git repository.
The canonical, version-controlled copy of this document lives here in
bolt-core-sdk.
...
| Item | Status |
|------|--------|
| `rust/bolt-core` v0.1.1 | Constants + vector generator + module stubs (R0 scaffold) |
| `constants` module | 8 constants, aligned with TS |
| `vectors` module | Box-payload + framing JSON generators (feature-gated) |
...
bolt-core-sdk/
  rust/
    bolt-core/                    # existing v0.1.1
      Cargo.toml
      src/
        lib.rs                    # re-exports
        constants.rs              # protocol constants (extracted from lib.rs)
        encoding.rs               # base64 + hex utilities (stub, R1)
        crypto.rs                 # seal_box_payload, open_box_payload (stub, R1)
        identity.rs               # generate_identity_keypair, KeyMismatchError (stub, R2)
        peer_code.rs              # generate, validate, normalize (partial, R3)
        sas.rs                    # compute_sas (stub, R2)
        hash.rs                   # sha256, buffer_to_hex (stub, R2)
        errors.rs                 # BoltError hierarchy (complete)
        vectors.rs                # existing vector generator (feature-gated, test-only)
      tests/
        vector_compat.rs          # existing (feature-gated)
        vector_equivalence.rs     # existing (feature-gated)
        crypto_parity.rs          # NEW in R1: seal/open against golden vectors
        sas_parity.rs             # NEW in R2: SAS against golden vectors
        identity_parity.rs        # NEW in R2: keypair generation properties
        peer_code_parity.rs       # NEW in R3: format, alphabet, validation
```
...
### Current (`Cargo.toml` v0.1.1, post-R0.1)

```toml
[dependencies]
thiserror = "2"

[dependencies] # optional, behind `vectors` feature
...
Existing `ci-rust.yml` runs `cargo test` and `cargo test --features vectors`
which includes vector compat and equivalence tests. New parity tests (R1-R3)
are added to the same workflow. No new CI files needed.

---

## 7. Migration Phases

### Phase R0: Crate Restructure + CI Gate — COMPLETE

**Commit:** `cb17b6a` (scaffold), `ae2477a` (dependency correction)

Extracted constants to own file, added 8 module stubs, feature-gated
vectors behind `vectors` cargo feature, removed unused sha2/rand deps.
CI tightened to `clippy -D warnings`. 23 unit tests (default), 28 total
(with vectors feature).
