...
envelope-open operations. Eliminates cross-implementation drift between
TS and Rust SDK. Vector files generated by deterministic script using
fixed keypairs and nonces. Open-only testing (no seal-output equality
since nonces are random in production).

### Added (TS — bolt-core)
- `scripts/generate-h3-vectors.mjs` — deterministic vector generator
  (fixed keypairs, `--check` mode for CI).
- `__tests__/vectors/sas.vectors.json` — 4 SAS cases.
- `__tests__/vectors/web-hello-open.vectors.json` — 3 HELLO open cases.
- `__tests__/vectors/envelope-open.vectors.json` — 3 envelope open cases.
- `__tests__/hello-open-vectors.test.ts` — HELLO open vector tests.
- `__tests__/envelope-open-vectors.test.ts` — envelope open vector tests.
- `__tests__/sas.test.ts` updated to load from `sas.vectors.json`.

### Added (Rust — bolt-core)
- `tests/sas_vectors.rs` — SAS golden vector tests (feature-gated `vectors`).
- `tests/h3_open_vectors.rs` — HELLO/envelope open vector tests, wrong-key
  rejection, seal-then-open round-trips (feature-gated `vectors`).

### Tests
- TS (bolt-core): 94 tests (9 test files).
- Rust: 68 tests (54 unit + 14 integration with `--features vectors`).

**Tag:** `sdk-v0.5.1`
**Commit:** `9d8617d`
**Branch:** `feature/h3-golden-vectors` (not yet merged to main)

## [H2 — WebRTC Enforcement Compliance] - 2026-02-25

Phase H2: Implement exactly-once HELLO, envelope-required binary
enforcement, and fail-closed semantics in WebRTCService per
PROTOCOL_ENFORCEMENT.md.

### Changed (bolt-transport-web)
- `WebRTCService` enforces exactly-once HELLO (duplicate triggers
  `DUPLICATE_HELLO` error + disconnect).
- Envelope-required mode: all post-HELLO messages rejected if not
  wrapped in Profile Envelope when `bolt.envelope` negotiated.
- Fail-closed: every protocol error triggers error frame + disconnect.
  No log-and-continue paths.
- Downgrade resistance: no runtime flag can disable enforcement
  post-negotiation.

### Added (bolt-transport-web)
- `webrtcservice-h2-enforcement.test.ts` — 21 enforcement tests
  covering exactly-once HELLO, envelope-required rejection, fail-closed
  for each error code, downgrade resistance.

### Tests
- bolt-transport-web: 138 tests (was 117, +21 H2 enforcement tests).

**Tag:** `sdk-v0.5.0-h2-webrtc-enforcement`
**Commit:** `b4ce544`
**Branch:** `feature/h3-golden-vectors` (not yet merged to main)
