...
/// AC-EN-10: Connection screen — real peer code, dynamic status, timeout handling.
/// Peer code from bolt_core::peer_code (runtime-generated).
/// Connect timeout prevents infinite hang (5s → TimedOut state).
pub fn show(
    ui: &mut egui::Ui,
    local_peer_code: &str,
    ...
    ui.vertical_centered(|ui| {
        ...
        let input = egui::TextEdit::singleline(peer_code_input)
            ...
            .interactive(!is_connecting); // Disable input while connecting
        ui.add(input);

        ui.add_space(theme::SPACING_LG);

        // Show Connect or Cancel button depending on state
        if is_connecting {
            ...
            let btn_label = if connection.can_retry()
                && !matches!(connection, ConnectionState::Disconnected)
            {
                "Retry"
            } else {
                "Connect"
            ...
            let btn_enabled = !peer_code_input.is_empty();
        ...
        let status_string = match connection {
            ...
            ConnectionState::TimedOut => {
                "Connection timed out — daemon not available".to_string()
            }
        ...
        // Show hint for timeout/error
        if matches!(
            connection,
            ConnectionState::TimedOut | ConnectionState::Error(_)
        ) {
            ui.add_space(theme::SPACING_SM);
            ui.label(
                egui::RichText::new("Daemon IPC not yet implemented — connect path requires bolt-daemon integration")
                    .size(theme::FONT_SIZE_SMALL)
                    .color(theme::TEXT_MUTED),
            );
        }
