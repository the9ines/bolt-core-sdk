...
/// AC-EN-11: Transfer screen — runtime state driven.
/// Progress, file info, and status from TransferState model.
pub fn show(ui: &mut egui::Ui, transfer: &TransferState) {
    ...
    ui.vertical_centered(|ui| {
        ...
        // Dynamic status from transfer state model
        ...
        // Send button — disabled when not connected (EN3 state-driven)
        ...
        // Disabled until daemon IPC is wired (EN3 partial — see evidence)
        if ui.add_enabled(false, send_btn).clicked() {
            // Requires daemon IPC for file picker + transfer initiation
        }
