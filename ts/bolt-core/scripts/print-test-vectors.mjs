/**
 * @deprecated AC-RC-09: This TS vector generator is DEPRECATED.
 * Canonical vectors are now generated by the Rust SDK.
 *
 * Use instead:
 *   cargo test -p bolt-core --features vectors --test vector_golden_core
 *
 * Canonical vector files: rust/bolt-core/test-vectors/core/
 *
 * This script is retained for transitional CI compatibility only.
 * It will be removed in a future release.
 *
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║  TEST FIXTURES ONLY — NEVER USE IN PRODUCTION                  ║
 * ║                                                                ║
 * ║  All keypairs in this script are deterministic, publicly known  ║
 * ║  test fixtures. They are NOT valid cryptographic material for   ║
 ...
 *   node scripts/print-test-vectors.mjs --check   # regen to temp + diff (CI use)
 *
...
const boxPayload = {
  _WARNING: 'TEST FIXTURES ONLY — NEVER USE IN PRODUCTION. All keypairs are deterministic, publicly known, and not valid cryptographic material for real deployments.',
...
const framing = {
  _WARNING: 'TEST FIXTURES ONLY — NEVER USE IN PRODUCTION. Sealed payloads use deterministic, publicly known test keypairs.',
...
if (checkMode) {
  // CI mode: compare generated vectors against committed files
  let mismatch = false;
  const pairs = [
    ['box-payload.vectors.json', boxPayload],
    ['framing.vectors.json', framing],
  ];
  for (const [filename, data] of pairs) {
    ...
    if (committed !== generated) {
      ...
      console.error('Run: npm run generate-vectors');
      mismatch = true;
    } else {
